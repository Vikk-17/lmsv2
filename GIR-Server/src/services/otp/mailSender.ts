...
interface SendApprovalMailParams {
  email: string;
  password: string;
  name: string;
}

interface SendRejectionMailParams {
  email: string;
  name: string;
}
...
const sendApprovalMail = async ({
  ...
  password,
  name,
}: SendApprovalMailParams): Promise<void> => {
  ...
  await transporter.sendMail({
    ...
    text: `Congratulations ${name}! We are delighted to inform you that your application for the instructor position at GIR Technologies has been approved. Your login credentials: Email: ${email}, Password: ${password}. Welcome to the team!`,
     html: `
...
const sendRejectionMail = async ({
  email,
  name,
}: SendRejectionMailParams): Promise<void> => {
  const transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: process.env.EMAIL_USER,
  ...
  await transporter.sendMail({
    ...
    text: `Dear ${name}, Thank you for your interest in the instructor position at GIR Technologies. After careful consideration, we regret to inform you that we will not be moving forward with your application at this time. We appreciate the time and effort you invested in the interview process.`,
    html: `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 650px; margin: 0 auto; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 0; border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.2);">
